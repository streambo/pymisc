...
class Trader():
	...
	def processOrder(self, dt, price, volume, notes=''):
		...
		if volume > 0:
			self.balance = self.balance - volume * (price + round(price * self.feeRate, 2))
			#self.stats['log'].append(str(self.balance))
			self.stats['buy']['date'].append(dt)
			self.stats['buy']['price'].append(price - 100)
			
		elif volume < 0:
			self.balance = self.balance - volume * (price - round(price * self.feeRate, 2))
			#self.stats['log'].append(str(self.balance))
			self.stats['sell']['date'].append(dt)
			self.stats['sell']['price'].append(price - 100)
	...
	def summary(self, dt, price):
		...
		if self.position < 0:
			self.equity = self.balance + self.position * (price + round(price * self.feeRate, 2))
		else:
			self.equity = self.balance + self.position * (price - round(price * self.feeRate, 2))
